...
async function fetchAllHaikus(params: {
  page: number;
  page_size: number;
  q?: string;
  ...
  const res = await fetch(`/api/haiku/all?${sp.toString()}`, {
    method: "GET",
    cache: "no-store",
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to fetch haikus (${res.status}): ${txt}`);
  }
...
export default function AllHaikuList() {
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const json = await fetchAllHaikus({
          page,
          page_size: pageSize,
          q: q || undefined,
          sort,
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between lg:mb-15">
        <div>
          ...
          <p className="text-sm text-teal-Deer">{data ? `${data.total} 件` : "—"}</p>
        </div>
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-center">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  // kalau mau input-nya nggak kosongin, hapus line di bawah
                  // setSearchInput("");
                }
              }}
          ...
          <div className="flex gap-2">
            ...
            <select
              value={pageSize}
              onChange={(e) =>
                setParams({ page_size: Number(e.target.value), page: 1 })
              }
              className="rounded-md border border-ateneo-blue bg-white px-3 py-2 text-sm text-ateneo-blue"
            >
              {[8, 12, 16].map((n) => (
      ...
      {loading && <div className="text-sm text-slate-500">ロード...</div>}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center mt-4 space-x-2">
          <button
            className={clsx(
              "rounded-md px-3 py-1 text-white hover:bg-black disabled:bg-gray-400",
              page === 1 ? "bg-ateneo-blue" : "bg-lime-green",
            )}
          ...
          {pageButtons[0] !== 1 && (
            <>
              <button
                className="rounded-md bg-ateneo-blue px-3 py-1 text-white hover:bg-black"
          ...
          {pageButtons.map((p) => (
            <button
              ...
              className={clsx(
                ...
                p === safePage ? "bg-ateneo-blue" : "bg-lime-green",
              )}
          ...
          {pageButtons[pageButtons.length - 1] !== totalPages && (
            <>
              ...
              <button
                className={clsx(
                  "rounded-md px-3 py-1 text-white hover:bg-black",
                  safePage === totalPages ? "bg-ateneo-blue" : "bg-lime-green",
                )}
          ...
          <button
            className="rounded-md bg-ateneo-blue px-3 py-1 text-white hover:bg-black disabled:bg-gray-400"
